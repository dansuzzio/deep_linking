...
class BeamerAppNavigationController implements AppNavigationState {
  ...
  BuildContext? get context => observer.navigator?.context;

  @override
  AppRoute get currentRoute {
    final path = context?.currentBeamLocation.state.routeInformation.location;
    return getRouteForPath(path);
  }

  @override
  void goTo(AppRoute route) {
    if (context case final context?) return context.beamToNamed(route.path);
